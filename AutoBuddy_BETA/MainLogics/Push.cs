...
namespace Buddy_vs_Bot.MainLogics
{
    internal class Push
    {
        ...
        public void Reset(Obj_AI_Base myTower, Obj_AI_Base enemyTower, Lane ln)
        {
            Vector3 pingPos = AutoWalker.p.Distance(AutoWalker.myNexus) - 100 > myTower.Distance(AutoWalker.myNexus)
                ? enemyTower.Position
                : myTower.Position;
            Core.DelayAction(() => TacticalMap.SendPing(PingCategory.OnMyWay, new Vector3(pingPos.X + RandGen.r.NextFloat(-300, 300), pingPos.Y + RandGen.r.NextFloat(-300, 300), pingPos.Z)), RandGen.r.Next(3000));
            lane = ln;
            currentWave = new Obj_AI_Minion[0];
            myTurret = myTower;
        ...
        void Drawing_OnDraw(EventArgs args)
        {
            ...
            Drawing.DrawText(250, 40, System.Drawing.Color.Gold, "Push, active: " + active + "  wave num: " + CurrentWaveNum+ " minions left: " + currentWave.Length );
                    ...
                    p =
                        p.Extend(p.Extend(
                            AutoWalker.p.Distance(myTurret) < AutoWalker.p.Distance(enemyTurret)
                                ? myTurret
                                : enemyTurret,
                            400).To3D().RotateAround(p, 1.57f), randomExtend).To3DWorld();
                }
                AutoWalker.WalkTo(p);
            }
            else
    ...
    EntityManager.Heroes.Allies.Where(
        ...
        .FirstOrDefault();
            if (ally != null)
                p = ally.Position.Extend(myTurret, 160).To3DWorld() + randomVector;
            Orbwalker.ActiveModesFlags = AutoWalker.p.Distance(enemyTurret) < 900 ? Orbwalker.ActiveModes.LastHit : Orbwalker.ActiveModes.LaneClear;
            AutoWalker.WalkTo(ally==null?myTurret.Position.Extend(AutoWalker.p.Position, 350+randomExtend/2).To3D().RotateAround(myTurret.Position, randomAngle):p );
        }
        ...
        {
            Core.DelayAction(SetWaveNumber, 500);
            Obj_AI_Minion closest = ObjectManager.Get<Obj_AI_Minion>().Where(min => min.IsAlly&&min.Name.Length>13&&min.GetLane()==lane&&min.HealthPercent>80).OrderBy(min=>min.Distance(enemyTurret)).FirstOrDefault();
            if (closest != null)
            {
        ...
        private void SetCurrentWave()
        {
            ...
            if (!wholeWave && newMinions.Length < 7)
            {
                ...
                Core.DelayAction(SetCurrentWave, newMinions.Any(min => min.Distance(AutoWalker.myNexus) < 800) ? 3000 : 300);
            }
            else
            {
                wholeWave = false;
        ...
        private void SetCurrentWaveRework()
        {
            if (CurrentWaveNum == 0)
            {
                Core.DelayAction(SetCurrentWave, 1000);
